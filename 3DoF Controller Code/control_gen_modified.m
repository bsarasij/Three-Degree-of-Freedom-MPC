function uk=control_gen_modified(pH_k, CO_2_k_1, Dm_curr, filt_ref,...
...
persistent modparms narx_reg_struct big_reg_pH big_reg_CO_2...
    big_reg_Temp big_reg_Rad x_hat_k_given_k x_hat_k_given_k_filt %arx_model

% Load modparams
if isempty(modparms)
    out_modparms=load('modparms_08_6_ms.mat');
    modparms=out_modparms.modparms_08_6_ms;
end
...
% end
if isempty(narx_reg_struct)
    out_narx=load('arx446.mat');
    narx_reg_struct=out_narx.arx446;
end


% if isempty(arx_model)
%     out_arx_model=load('arx446_reg.mat');
%     arx_model=out_arx_model.arx446_reg;
% end
...
if isempty(big_reg_Rad)
    ...
    x_init=zeros(9+2*n_out,1); %zeros(sum(NA(1,:))+sum(NB(1,:))+2*n_out-1,1); % Defining Intitial (Steady-State) Conditions
...
elseif model_choice==0
    ...
    idpol=idpoly(id_A_arx,id_B_arx,'Ts',ts);%arx_model;
    ss_mod_orig=ss(idpol);
    ss_mod_orig.InputDelay=NK'-1;
    ss_model=ssDel2ABCD(ss_mod_orig);
end
...
[uk, x_hat_k_given_k, x_hat_k_given_k_filt] = Hyb_Mpc44_bioreactor(...
    x_hat_k_minus_1_given_k_minus_1, y_plant_measurement, uk_minus_1, ...
    delta_uk_minus_1, Dm_minus1, Dm_minus2, Dm_filt_minus1,...
    Dm_filt_minus2, D_forecast_filt, D_forecast_unfilt, filt_ref,...
    deltak_minus_1, zk_minus_1, delta_deltak_minus_1, delta_zk_minus_1,...
    x_hat_k_minus_1_given_k_minus_1_filt, HybMpcCon, degree, slack_var);
